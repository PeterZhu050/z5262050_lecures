...
def mk_ret_df(prc_df):
    ...
    # <COMPLETE THIS PART>
    ff_daily = pd.read_csv(cfg.FF_CSV)
    ff_daily['Date'] = pd.to_datetime(ff_daily['Date'], format = '%Y-%m-%d')
    prc_df = prc_df.sort_index()
    returns_df = pd.DataFrame(index = prc_df.index)

    stocks = prc_df.columns
    for stock in stocks:
        returns_df[stock] = (prc_df[stock] - prc_df[stock].shift(1))/prc_df[stock].shift(1)

    mkt = []
    for i, date in enumerate(returns_df.index):
        mkt.append(ff_daily.loc[ff_daily.Date == date, 'mkt'].values[0])
    returns_df['mkt'] = mkt
...
def mk_aret_df(ret_df):
    ...
    abreturns_df = pd.DataFrame(index = ret_df.index)

    stocks = ret_df.columns
    for stock in stocks[:-1]:
        abreturns_df[stock] = ret_df[stock] - ret_df['mkt']
